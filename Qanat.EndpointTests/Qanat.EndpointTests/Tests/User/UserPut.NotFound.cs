...
namespace Qanat.EndpointTests.Tests.User
{
    ...
    {
        [DataTestMethod]
        public async ThreadingTask CannotPutWithInvalidUserID()
        {
            var users = await TestHelper.GetUsers(_validTokenWithPermission);
            Assert.IsNotNull(users);
            var user = users.First();

            var route = $"users/{int.MaxValue}";
            ...
            var result = await APIHelper.Put<UserDto>(_validTokenWithPermission, route, JsonConvert.SerializeObject(userUpsertDto));
