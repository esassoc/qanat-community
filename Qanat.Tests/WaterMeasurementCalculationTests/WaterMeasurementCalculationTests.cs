...
namespace Qanat.Tests.WaterMeasurementCalculationTests;
...
public class WaterMeasurementCalculationTests
{
    ...
    public async Task MIUGSA_CanUpdateETPrecipAndHaveDependantCalculationsModified(string dateAsString, string usageEntityName, double updatedAmountInAcreFeetAsDouble)
    {
        ...
        //#region Reset Back to Previous Values

        ////In order to have this test be repeatable without a restore we have to reset the precip back to what it was before.
        //etPrecipMeasurement.ReportedValue = previousETPrecipValueInInches;
        //etPrecipMeasurement.ReportedValueInAcreFeet = previousETPrecipValueInAcreFeet;

        //await _dbContext.SaveChangesAsync();

        //await WaterMeasurementCalculations.RunMeasurementTypeForGeography(_dbContext, geographyID, openETPrecipMeasurementTypeID, dateToCalculate);

        //var cleanedUpExtractedGroundwaterMeasurement = await _dbContext.WaterMeasurements.SingleOrDefaultAsync(x => x.GeographyID == geographyID && x.ReportedDate.Date == dateToCalculate.Date && x.WaterMeasurementTypeID == extractedGroundwaterMeasurementTypeID && x.UsageEntityName == usageEntityName);

        //Assert.IsNotNull(cleanedUpExtractedGroundwaterMeasurement);

        //var cleanedUpExtractedGroundwaterValueInAcreFeet = Math.Round(previousExtractedGroundwaterValueInAcreFeet, 4,MidpointRounding.ToEven);
        //var cleanedUpExtractedGroundwaterValueInInches = Math.Round(previousExtractedGroundwaterValueInInches, 4,MidpointRounding.ToEven);

        //Assert.IsTrue(Math.Abs(previousExtractedGroundwaterValueInAcreFeet - cleanedUpExtractedGroundwaterValueInAcreFeet) <= _acceptableTolerance, $"Value in Acre Feet do not match within tolerance. Expected: {previousExtractedGroundwaterValueInAcreFeet}. Got: {cleanedUpExtractedGroundwaterValueInAcreFeet}");
        //Assert.IsTrue(Math.Abs(previousExtractedGroundwaterValueInInches - cleanedUpExtractedGroundwaterValueInInches) <= _acceptableTolerance, $"Reported Value in Acre Feet do not match within tolerance. Expected: {previousExtractedGroundwaterValueInInches}. Got: {cleanedUpExtractedGroundwaterValueInInches}");
    ...
    public async Task MIUGSA_CanUpdateExtractedGroundwaterAdjustmentAndHaveDependantCalculationsModified(string dateAsString, string usageEntityName, double updatedAmountInAcreFeetAsDouble)
    {
        ...
        //Need to revert this value back or else CanRunDependencyFlowAndGetSameResult will start failing as the legacy flow can't know that the adjustment had already been applied.
        extractedGroundwaterAdjustment.ReportedValue = previousAdjustmentAmountInInches;
        extractedGroundwaterAdjustment.ReportedValueInAcreFeet = previousAdjustmentAmountInAcreFeet;

        await _dbContext.SaveChangesAsync();

        await WaterMeasurementCalculations.RunMeasurementTypeForGeography(_dbContext, geographyID, extractedGroundwaterAdjustmentMeasurementTypeID, dateToCalculate);

        var cleanedUpExtractedGroundwaterAdjustment = await _dbContext.WaterMeasurements.SingleOrDefaultAsync(x => x.GeographyID == geographyID && x.ReportedDate.Date == dateToCalculate.Date && x.WaterMeasurementTypeID == extractedGroundwaterAdjustmentMeasurementTypeID && x.UsageEntityName == usageEntityName);

        Assert.IsNotNull(cleanedUpExtractedGroundwaterAdjustment);
        Assert.AreEqual(0, cleanedUpExtractedGroundwaterAdjustment.ReportedValue);
        Assert.AreEqual(0, cleanedUpExtractedGroundwaterAdjustment.ReportedValueInAcreFeet);

        var cleanedUpExtractedGroundwater = await _dbContext.WaterMeasurements.SingleOrDefaultAsync(x => x.GeographyID == geographyID && x.ReportedDate.Date == dateToCalculate.Date && x.WaterMeasurementTypeID == extractedGroundwaterMeasurementTypeID && x.UsageEntityName == usageEntityName);

        Assert.IsNotNull(cleanedUpExtractedGroundwater);
        var cleanedUpValueInInches = Math.Round(previousExtractedGroundwaterValueInInches, 4,MidpointRounding.ToEven);
        var cleanedUpValueInAcreFeet = Math.Round(previousExtractedGroundwaterValueInAcreFeet, 4,MidpointRounding.ToEven);

        Assert.IsTrue(Math.Abs(previousExtractedGroundwaterValueInInches - cleanedUpValueInInches) <= _acceptableTolerance, $"Reported Value in Inches do not match within tolerance. Expected: {previousExtractedGroundwaterValueInInches}. Got: {cleanedUpValueInInches}");
        Assert.IsTrue(Math.Abs(previousExtractedGroundwaterValueInAcreFeet - cleanedUpValueInAcreFeet) <= _acceptableTolerance, $"Reported Value in Acre Feet do not match within tolerance. Expected: {previousExtractedGroundwaterValueInAcreFeet}. Got: {cleanedUpValueInAcreFeet}");
    ...
    public async Task ETSGSA_CanUpdateLandIQPrecipAndHaveDependantCalculationsModified(string dateAsString, string usageEntityName, double updatedAmountInAcreFeetAsDouble)
    {
        ...
        #region Reset to Previous Values

        //In order to have this test be repeatable without a restore we have to reset the precip back to what it was before.
        landIQPrecipMeasurement.ReportedValue = previousLandIQPrecipValueInInches;
        landIQPrecipMeasurement.ReportedValueInAcreFeet = previousLandIQPrecipValueInAcreFeet;

        await _dbContext.SaveChangesAsync();

        await WaterMeasurementCalculations.RunMeasurementTypeForGeography(_dbContext, geographyID, landIQPrecipMeasurementTypeID, dateToCalculate);

        var cleanedUpEffectivePrecipMeasurement = await _dbContext.WaterMeasurements.SingleOrDefaultAsync(x => x.GeographyID == geographyID && x.ReportedDate.Date == dateToCalculate.Date && x.WaterMeasurementTypeID == effectivePrecipMeasurementTypeID && x.UsageEntityName == usageEntityName);

        Assert.IsNotNull(cleanedUpEffectivePrecipMeasurement);

        var cleanedUpEffectivePrecipValueInAcreFeet = Math.Round(previousEffectivePrecipValueInAcreFeet, 4,MidpointRounding.ToEven);
        var cleanedUpEffectivePrecipValueInInches = Math.Round(previousEffectivePrecipValueInInches, 4,MidpointRounding.ToEven);

        Assert.IsTrue(Math.Abs(previousEffectivePrecipValueInAcreFeet - cleanedUpEffectivePrecipValueInAcreFeet) <= _acceptableTolerance, $"Value in Acre Feet do not match within tolerance. Expected: {previousEffectivePrecipValueInAcreFeet}. Got: {cleanedUpEffectivePrecipValueInAcreFeet}");
        Assert.IsTrue(Math.Abs(previousEffectivePrecipValueInInches - cleanedUpEffectivePrecipValueInInches) <= _acceptableTolerance, $"Reported Value in Acre Feet do not match within tolerance. Expected: {previousEffectivePrecipValueInInches}. Got: {cleanedUpEffectivePrecipValueInInches}");
    ...
    public async Task ETSGSA_CanUpdateSurfaceWaterAndHaveDependantCalculationsModified(string dateAsString, string usageEntityName, double updatedAmountInAcreFeetAsDouble, int surfaceWaterMeasurementTypeID)
    {
        ...
        #region Reset to Previous Value

        surfaceWaterMeasurement.ReportedValue = previousSurfaceWaterValueInInches;
        surfaceWaterMeasurement.ReportedValueInAcreFeet = previousSurfaceWaterValueInAcreFeet;
