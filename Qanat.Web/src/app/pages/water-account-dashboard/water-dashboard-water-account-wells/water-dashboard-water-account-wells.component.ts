...
@Component({
  selector: 'water-dashboard-water-account-wells',
  templateUrl: './water-dashboard-water-account-wells.component.html',
  styleUrls: ['./water-dashboard-water-account-wells.component.scss'],
  standalone: true,
  imports: [NgIf, PageHeaderComponent, ModelNameTagComponent, RouterLink, QanatGridComponent, AsyncPipe, QanatMapComponent, WellsLayerComponent],
})
export class WaterDashboardWaterAccountWellsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.waterAccount$ = this.route.paramMap.pipe(
      ...
      tap((waterAccount) => {
        ...
        this.wells$ = this.route.paramMap.pipe(
          switchMap(() => this.wellService.waterAccountsWaterAccountIDWellsGet(waterAccount.WaterAccountID)),
        );
      })
    );
