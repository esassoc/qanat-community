...
import { GeographyMinimalDto, GeographyPublicDto } from "src/app/shared/generated/model/models";
...
@Component({
    ...
    standalone: true,
    imports: [PageHeaderComponent, RouterLink, AlertDisplayComponent, NgIf, QanatGridComponent, FormsModule, AsyncPipe, DecimalPipe, LoadingDirective],
...
export class ReviewSelfReportComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.selfReports$ = this.geography$.pipe(
            switchMap((geography) =>
                this.waterMeasurementReviewSelfReportService.geographiesGeographyIDWaterMeasurementSelfReportsGet(geography.GeographyID).pipe(
                    tap((reports) => {
                        this.submittedCount = reports.filter((report) => report.WaterMeasurementSelfReportStatusID == WaterMeasurementSelfReportStatusEnum.Submitted).length;
                        this.approvedCount = reports.filter((report) => report.WaterMeasurementSelfReportStatusID == WaterMeasurementSelfReportStatusEnum.Approved).length;
                    })
                )
            )
        );
