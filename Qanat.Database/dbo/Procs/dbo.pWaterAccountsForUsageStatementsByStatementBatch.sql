create procedure dbo.pWaterAccountsForUsageStatementsByStatementBatch
(
	@statementBatchID int
)
as
...
select wa.WaterAccountID, wa.WaterAccountNumber, wac.ContactName,  wac.[Address], wac.SecondaryAddress, wac.City, wac.[State], wac.ZipCode, wa.WaterAccountPIN,
	...
	rp.StartDate as ReportingPeriodStartDate, rp.EndDate as ReportingPeriodEndDate,
...
join dbo.StatementBatchWaterAccount sbwa on wa.WaterAccountID = sbwa.WaterAccountID
join dbo.StatementBatch sb on sbwa.StatementBatchID = sb.StatementBatchID
join dbo.ReportingPeriod rp on sb.ReportingPeriodID = rp.ReportingPeriodID
join dbo.Geography g on sb.GeographyID = g.GeographyID
join (
	...
	join dbo.StatementBatch sb on wap.ReportingPeriodID = sb.ReportingPeriodID 
	join dbo.GeographyAllocationPlanConfiguration gapc on sb.GeographyID = gapc.GeographyID
	...
	) ula on p.ParcelID = ula.ParcelID and sb.ReportingPeriodID = ula.ReportingPeriodID
	where sb.StatementBatchID = @statementBatchID
