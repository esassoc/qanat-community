...
[ApiController]
[RightsChecker]
public class WellRegistrationController(QanatDbContext dbContext, ILogger<WellRegistrationController> logger, IOptions<QanatConfiguration> qanatConfiguration)
    : SitkaController<WellRegistrationController>(dbContext, logger, qanatConfiguration)
{
    ...
    public ActionResult<List<WellRegistrationGridRowDto>> List([FromRoute] int geographyID)
    {
        var wellRegistrationGridRowDtos = WellRegistrations.ListByGeographyID(_dbContext, geographyID);
        return Ok(wellRegistrationGridRowDtos);
    }
    ...
    [EntityNotFound(typeof(WellRegistration), "wellRegistrationID")]
    [WithWellRegistrationOwnerContextPermission]
    public ActionResult Submit([FromRoute] int wellRegistrationID)
    {
        var wellWorkflow = WellRegistrations.GetByIDWithTrackingForWorkflow(_dbContext, wellRegistrationID)
            .GetWorkflow(_dbContext, UserContext.GetUserFromHttpContext(_dbContext, HttpContext));
