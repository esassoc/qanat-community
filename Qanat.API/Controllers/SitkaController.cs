...
public abstract class SitkaController<T> : ControllerBase
{
    ...
    protected ActionResult RequireNotNullLogIfNotFound(object theObject, string objectType, object objectID)
    {
        return CheckAndLogIfNotFound(theObject, objectType, objectID, out var actionResult) ? actionResult : Ok(theObject);
    }

    protected bool CheckAndLogIfNotFound(object theObject, string objectType, object objectID, out ActionResult actionResult)
    {
        if (theObject == null)
        {
