...
public class WellController : SitkaController<WellController>
{
    ...
    public ActionResult<WellDetailDto> GetWellAsDetailDto([FromRoute] int wellID)
    {
        ...
        var wellDetailDto = new WellDetailDto()
        {
            ...
            WellType = new WellTypeMinimalDto()
            {
                WellTypeID = well.WellType.WellTypeID,
                Name = well.WellType.Name,
                HasSchomotoSchema = !string.IsNullOrEmpty(well.WellType.SchemotoSchema)
            },
    ...
    [HttpGet("wells/{wellID}/instance")]
    [EntityNotFound(typeof(Well), "wellID")]
    [WithGeographyRolePermission(PermissionEnum.WellRights, RightsEnum.Read)]
    public async Task<ActionResult<WellInstanceDto>> GetWellInstance([FromRoute] int wellID)
    {
        var wellInstanceDto = await Wells.GetInstanceAsync(_dbContext, wellID);
        return Ok(wellInstanceDto);
    }
    ...
    [HttpPut("wells/{wellID}/parcel")]
    [EntityNotFound(typeof(Well), "wellID")]
    [WithGeographyRolePermission(PermissionEnum.WellRights, RightsEnum.Update)]
    public ActionResult<WellMinimalDto> OverrideWellParcel([FromRoute] int wellID, [FromBody] WellParcelDto wellParcelDto)
    {
        if (wellID != wellParcelDto.WellID)
        {
            return BadRequest();
        }

        var errors = Wells.ValidateWellParcelOverride(_dbContext, wellParcelDto);
        if (errors.Any())
        {
        ...
        Wells.OverrideWellParcel(_dbContext, wellParcelDto);

        var wellMinimalDto = Wells.GetByID(_dbContext, wellID).AsMinimalDto();
        return Ok(wellMinimalDto);
    }
    ...
    public async Task<ActionResult> UploadGDBAndParseFeatureClasses([FromForm] UploadedGdbRequestDto uploadedGdbRequestDto, [FromRoute] int geographyID)
    {
        ...
        try
        {
            ...
            var parcelGeometries = await ParcelGeometries.ListByGeographyID(_dbContext, geographyID);
            foreach (var wellStaging in wellStagings)
            {
                //Get the 4326 geometry of the well point
                var wellLocation4326 = wellStaging.Geometry.ProjectTo4326(null);

                //Find the parcel that contains the well point
                var defaultParcel = parcelGeometries.FirstOrDefault(p => p.GeometryNative.Covers(wellStaging.Geometry));

                var well = new Well
                {
                    GeographyID = geographyID,
                    ParcelID = defaultParcel?.ParcelID,
                    WellName = wellStaging.WellName?.Trim(),
                    LocationPoint4326 = wellLocation4326,
