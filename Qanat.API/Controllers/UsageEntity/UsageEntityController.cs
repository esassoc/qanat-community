...
public class UsageEntityController : SitkaController<UsageEntityController>
{
    ...
    [WithWaterAccountRolePermission(PermissionEnum.UsageEntityRights, RightsEnum.Read)]
    public async Task<ActionResult<List<UsageEntityListItemDto>>> GetUsageEntityByParcelID([FromRoute] int parcelID)
    {
        var usageEntities = await UsageEntities.GetListByParcelID(_dbContext, parcelID);
